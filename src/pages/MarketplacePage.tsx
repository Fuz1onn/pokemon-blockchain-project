...
interface MarketplaceProps {
  marketplacePokemons: Pokemon[];
  setMarketplacePokemons: React.Dispatch<React.SetStateAction<Pokemon[]>>;
...
const MarketplacePage: React.FC<MarketplaceProps> = ({
  ...
  const fetchRandomPokemons = async (count = 8) => {
    ...
    setMarketplacePokemons((prev) => [...prev, ...fetched]);
    setLoading(false);
  };

  // Initial fetch
  useEffect(() => {
    if (marketplacePokemons.length === 0) {
      fetchRandomPokemons();
    }
  }, []);
  ...
  // Apply filters
  const filteredPokemons = marketplacePokemons
    .filter((p) => {
  ...
  if (loading && marketplacePokemons.length === 0)
    return (
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-6">
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        ...
        <button
          ...
          className="bg-gray-800 border border-gray-600 rounded-lg p-2 px-4 text-white cursor-pointer hover:bg-gray-700"
        >
          Reset
        </button>
      </div>
